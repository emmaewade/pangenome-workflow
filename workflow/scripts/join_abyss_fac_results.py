...
#big table
joined = pd.read_csv(args.master ,dtype='unicode', sep="\t")
joined.columns = ["assembly_accession_base", "assembly_accession", "asm_name", "taxid", "organism_name", "infraspecific_name", "version_status", "assembly_level", "ftp_path"]
#abyss-fac table
abyss = pd.read_table(args.abyss_out)
abyss['name_cut'] = abyss['name'].str.slice(2, 17)
abyss = abyss[~abyss['name'].str.contains('name')]
abyss_join = pd.merge(joined, abyss, left_on = 'assembly_accession', right_on='name_cut', how = 'outer')
